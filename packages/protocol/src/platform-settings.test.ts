...
import { DEFAULT_DISPLAY_SETTINGS, displaySettingsSchema, resolveWaitingVideoUrl, waitingVideoUrlSchema } from "./platform-settings.js";

describe("waiting display settings", () => {
  it("loads existing saved text without requiring a migration", () => {
    const { waitingVideoUrl, groupWaitingVideoUrls, ...legacy } = DEFAULT_DISPLAY_SETTINGS;
