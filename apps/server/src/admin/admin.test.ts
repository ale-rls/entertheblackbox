...
function setup(options: {
  ...
  const engine = {
    ...
    connectedDisplays: [{ kind: "main", id: null, heartbeatAgeMs: 12 }, { kind: "group", id: "a", heartbeatAgeMs: 30 }],
...
describe("admin API", () => {
  ...
  it("protects every admin endpoint and exposes operational status", async () => {
    ...
    expect(response.json()).toMatchObject({
      ...
      displays: [
        { kind: "main", id: null, label: null, heartbeatAgeMs: 12 },
        { kind: "group", id: "a", label: "a", heartbeatAgeMs: 30 },
      ],
      mainDisplayNeeded: false,
