...
describe("trackingbox positions", () => {
  ...
  it("attributes a claimed body's position to the phone, not to gid:<n>", async () => {
    ...
    // Same physical body throughout, positioned once before the claim and once
    // after. Before it votes anonymously as gid:42; after, as phone-1. The
    // claim moves the earlier vote across, so the count stays at one person.
    h.engine.applyTrackingActions([
      { type: "join", gid: 42 },
      { type: "position", gid: 42, x: 0.8, y: 0.2 },
    ]);
    h.engine.claimTrackedBody("phone-1", 42, h.now());
    expect(h.engine.bindings.participantForGid(42)).toBe("phone-1");
    h.engine.applyTrackingActions([{ type: "position", gid: 42, x: 0.8, y: 0.2 }]);

    await Promise.resolve();
    h.advance(20_000);
    // One physical body, one vote. The anonymous vote cast before the claim
    // moves to the phone rather than being counted a second time.
    expect(last(display, "question_resolved")).toMatchObject({
      winner: "q1",
      quadrantCounts: { q1: 1 },
    });
