...
export function PreviewPanel({ project, onClose }: { project: StudioProject; onClose: () => void }) {
  ...
  const outcomes: ForcedOutcome[] = phase.kind === "position-question" || phase.kind === "video-position-question"
  ...
  return <main className="preview-shell" data-sc-tool-density="compact" data-sc-tool-root><section className="preview" aria-label="Show preview"><header><div><p className="sc-tool-eyebrow">Simulation</p><h2>Outcome preview</h2></div><button className="sc-tool-button" data-sc-tool-variant="secondary" onClick={onClose}>Close preview</button></header>
    ...
    {phase.kind === "video" && <><p>Media placeholder: {phase.src}{phase.audioSrc ? ` + ${phase.audioSrc}` : ""}{phase.extraAudioSrc ? ` + extra ${phase.extraAudioSrc}` : ""}</p><button className="sc-tool-button" data-sc-tool-variant="secondary" onClick={() => setSession((value) => advanceTimer(value, phase.expectedDurationMs))}>Advance expected timer</button><button className="sc-tool-button" data-sc-tool-variant="secondary" onClick={() => setSession(advancePreview)}>Next phase</button></>}
    {phase.kind === "video-position-question" && <><p>Media placeholder: {phase.src}{phase.audioSrc ? ` + ${phase.audioSrc}` : ""}{phase.extraAudioSrc ? ` + extra ${phase.extraAudioSrc}` : ""}</p><p>{phase.text}</p><p>Show {phase.showAtMs} ms · open {phase.openAtMs} ms · close {phase.closeAtMs} ms · hide {phase.hideAtMs} ms · media end {phase.expectedDurationMs} ms</p><button className="sc-tool-button" data-sc-tool-variant="secondary" onClick={() => setSession((value) => advanceTimer(value, Math.max(0, phase.closeAtMs - value.elapsedMs)))}>Advance to vote close</button><label className="sc-tool-checkbox"><input type="checkbox" checked={includeStale} onChange={(event) => setIncludeStale(event.target.checked)} /> Include stale</label><label className="sc-tool-checkbox"><input type="checkbox" checked={includeDisconnected} onChange={(event) => setIncludeDisconnected(event.target.checked)} /> Include disconnected</label><div className="preview-outcomes">{outcomes.map((outcome) => <button className="sc-tool-button" data-sc-tool-variant="secondary" key={outcome} onClick={() => force(outcome)}>{phase.next.type === "fixed" ? "Resolve fixed" : `Force ${outcome}`}</button>)}</div></>}
    {phase.kind === "idle" && <p>Idle/end phase reached.</p>}
    {phase.kind === "group-branch" && <>
      <p>Main display inactive. Preview one group path at a time, or jump to the shared reunion.</p>
      {phase.branches.map((branch) => <button key={branch.groupId} className="sc-tool-button" onClick={() => setSession((value) => advancePreview(value, branch.groupId))}>
        Follow {project.scenario.groups?.find((group) => group.id === branch.groupId)?.label ?? branch.groupId}
      </button>)}
      <button className="sc-tool-button" onClick={() => setSession((value) => advancePreview(value))}>Rejoin all groups</button>
    </>}
