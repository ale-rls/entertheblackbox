...
Source plan: `entertheblackbox-installation-implementation-plan.md` (v1, 2026-07-11).
...
- verify: pnpm -r typecheck && pnpm -r test → PASS (7 workspaces, node 22.17 / pnpm 9.12.2, 2026-07-11); pnpm --filter '@entertheblackbox/*' build → PASS (2026-07-11)
- reviewer: codex
...
- verify: `pnpm --filter @entertheblackbox/server typecheck` → PASS; focused cursor/engine/admission suites → PASS (21 tests); `pnpm -r typecheck` → PASS (7 workspaces); full server suite → 31/32 PASS with only the pre-existing sandbox-blocked localhost WebSocket test (`listen EPERM 127.0.0.1`), 2026-07-12.
- reviewer: claude
...
- verify: `pnpm --filter @entertheblackbox/server exec vitest run src/admission/qr.test.ts src/engine/phase-engine.test.ts` → PASS (15 tests); `pnpm --filter @entertheblackbox/server typecheck` → PASS; `pnpm -r typecheck` → PASS (7 workspaces); full server suite → 36/37 PASS with only the pre-existing sandbox-blocked localhost WebSocket test (`listen EPERM 127.0.0.1`), 2026-07-12.
- reviewer: none
...
- verify: `pnpm --filter @entertheblackbox/server exec vitest run src/engine/video.test.ts src/engine/phase-engine.test.ts` → PASS (16 tests); `pnpm --filter @entertheblackbox/server typecheck` → PASS; full server suite → 41/42 PASS with only the pre-existing sandbox-blocked localhost WebSocket test (`listen EPERM 127.0.0.1`), 2026-07-12.
- reviewer: claude
...
- verify: focused integration suite → PASS (3 tests); `pnpm --filter @entertheblackbox/server typecheck` → PASS; `pnpm -r typecheck` → PASS (7 workspaces); full server suite → 45/46 PASS with only the pre-existing sandbox-blocked localhost WebSocket test (`listen EPERM 127.0.0.1`), 2026-07-12.
- reviewer: claude
...
- notes: MediaStore (injectable caches/fetch/digest): boot sync keyed by content hash, byte+sha256 verify per download, stale-entry eviction, endless capped-backoff retry (never ready with missing media); Blob URLs memoized per src, retainOnly() revokes outside the active set; useMedia hook gates UI on ready + visible retry state; public/sw.js is app-shell only (never intercepts /media/), cache-first hashed assets, network-first HTML. Next-video preload wired but waits on STEP-026 (id→src map) — only the active video gets a Blob URL until then. FIXES 2026-07-12: cached entries now re-hashed at boot (corruption test added); showVideo race purges stray Blob URLs via retainOnly on stale resolve; pending video re-resolves when sync reaches ready; SW narrowed to /assets/ + navigations only. Back to review. FIX 2026-07-12b: stop() now revokes all live Blob URLs (test added). Back to review.
  ...
  Verification by codex: `pnpm --filter @entertheblackbox/display typecheck` PASS; `pnpm --filter @entertheblackbox/display test` PASS (18 tests, 2 files); `pnpm --filter @entertheblackbox/display build` PASS; `node --check apps/display/public/sw.js` PASS, 2026-07-12. Status remains `review` pending the requested fixes.
...
- notes: CursorField (pure): 100 ms render-delay interpolation between last two ticks, stale-tick rejection, absent-cursor removal, join-halo progress, freeze semantics (ingest ignored while frozen); CursorCanvas rAF w/ destination-out fade trails, crisp full-clear while frozen; QuadrantOverlay pins q1 TR/q2 TL/q3 BL/q4 BR, live counts only when server sends them, winner/tie/empty highlight; store: question_status/resolved gated to current session+epoch, cleared on phase advance; cursors bypass React (direct CursorField ingest). freezeUntil display-side hold ends when server advances phase (server owns freezeMs timing). FIX 2026-07-12: freeze now derives from the reducer's session/epoch-gated resolution state via effect (stale question_resolved can no longer freeze the field); onMessage freeze shortcut removed. Back to review.
  ...
  Verification by codex: `pnpm --filter @entertheblackbox/display exec vitest run --reporter=verbose` PASS (27 tests / 3 files); `pnpm --filter @entertheblackbox/display typecheck` PASS; `pnpm --filter @entertheblackbox/display build` PASS, 2026-07-12. Status remains `review` pending the requested fix.
...
- verify: pnpm --filter @entertheblackbox/display typecheck && pnpm --filter @entertheblackbox/display test && pnpm --filter @entertheblackbox/display build → all PASS (40 tests incl. 12 new: 8 qr + 4 heartbeat) (2026-07-12)
- reviewer: none
...
- verify: Review-fix verification 2026-07-12: `pnpm --filter @entertheblackbox/server exec vitest run src/admin/admin.test.ts src/persistence/persistence.test.ts src/engine/phase-engine.test.ts` → PASS (28 tests); `pnpm --filter @entertheblackbox/server typecheck` → PASS; `git diff --check -- apps/server/src/engine/phase-engine.test.ts steps.md` → PASS. Original verification: focused server suites PASS (22 tests); admin test/build PASS; monorepo typecheck PASS; full server suite 54/55 with only sandbox-blocked localhost bind.
- reviewer: claude
...
- verify: `pnpm --filter @entertheblackbox/server exec vitest run src/server.test.ts -t "public video phase|invalid scenario"` → PASS (2 tests; 3 unrelated skipped); `pnpm --filter @entertheblackbox/server typecheck` → PASS, 2026-07-12. Node 20.12.2 emitted the expected repo engine warning for >=22.
- reviewer: none
...
- verify: `BUILD_VERSION=e2e-test-marker-42 pnpm --filter @entertheblackbox/<display|phone> build` → marker present in both dist bundles (grep 1 hit each); rebuild without env → "0.0.0-dev" fallback present in both; display+phone typecheck PASS; display test 40/40, phone test 11/11 PASS, 2026-07-12.
- reviewer: codex
...
- verify: `pnpm --filter @entertheblackbox/server typecheck` PASS; server tests 61/61 PASS (localhost-binding lifecycle test rerun with sandbox approval after expected EPERM); focused admission tests 10/10 PASS; `git diff --check` PASS, 2026-07-12.
- reviewer: none
...
- verify: `pnpm --filter @entertheblackbox/server typecheck` PASS; full server suite 63/63 PASS (localhost binding approved); focused persistence/runtime/server suite 17/17 PASS; `git diff --check` PASS, 2026-07-12.
- reviewer: claude
...
- verify: `pnpm --filter @entertheblackbox/server typecheck` PASS; focused persistence/runtime/server suite 20/20 PASS with localhost binding approved; full server suite 66/66 PASS before the final review-only lifecycle hardening (affected focused suites rerun after); `git diff --check` PASS, 2026-07-12.
- reviewer: claude
...
- acceptance: plan §5/§15/§17-A — parseRuntimeScenario()/compileStudioGraph()/validateStudioProject() skeletons over the REAL implemented schema (video next is a plain string, question next is a discriminated object; scenario-level cyclesAllowed; manifest is src/bytes/hash only); unknown-field policy decided and implemented (passthrough or raw-carry — never silently dropped, per plan §15); round-trip fixture tests: content/scenarios/dev.json + manifest → import → no edits → export is semantically identical after canonical normalization; compile ends by invoking the existing @entertheblackbox/scenario validator; docs/studio-compat.md records the compatibility matrix and versioning behavior
